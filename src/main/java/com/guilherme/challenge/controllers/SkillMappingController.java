...
public class SkillMappingController {
	...
	public ResponseEntity<List<SkillMappingDTO>> findAllSkills() {
		...
		skillMappingService.findAll()
				.forEach(skillMapping -> skillMappingDTOList.add(converter.convertToDTO(skillMapping)));
	...
	public ResponseEntity<List<SkillMappingDTO>> findSkillMappingByInterviewId(@PathVariable("id") String id) {
		...
		skillMappingService.findSkillMappingByInterview(interview)
				.forEach(skillMapping -> skillMappingDTOList.add(converter.convertToDTO(skillMapping)));
		return ResponseEntity.ok(skillMappingDTOList);
	}
	...
	public ResponseEntity<List<SkillMappingDTO>> findSkillMappingBySkillId(@PathVariable("id") String id) {
		...
		skillMappingService.findSkillMappingBySkill(skill)
				.forEach(skillMapping -> skillMappingDTOList.add(converter.convertToDTO(skillMapping)));
		return ResponseEntity.ok(skillMappingDTOList);
	}

	@GetMapping(value = "/professional/{id}")
	public ResponseEntity<List<SkillMappingDTO>> findSkillMappingByProfessionalId(@PathVariable("id") String id) {

		long idSkill = Long.valueOf(id);
		List<SkillMappingDTO> skillMappingDTOList = new ArrayList<SkillMappingDTO>();
		Professional professional = professionalService.findProfessionalByID(idSkill);

		skillMappingService.findSkillMappingByProfessional(professional)
				.forEach(skillMapping -> skillMappingDTOList.add(converter.convertToDTO(skillMapping)));
		return ResponseEntity.ok(skillMappingDTOList);
	}

	@PostMapping
	public ResponseEntity<SkillMappingDTO> saveSkill(@Valid @RequestBody SkillMappingDTO skillMappingDTO,
	...
	@PutMapping
	public ResponseEntity<SkillMappingDTO> updateSkill(@Valid @RequestBody SkillMappingDTO skillMappingDTO,
			BindingResult result) throws APIException {
		if (result.hasErrors()) {
		...
		if (!skillMappingService.existsSkillMappingWithInterviewAndSkill(skillMappingDTO.getInterview(),
			...
			throw new APIException(
					"No Skill Mapping was found with the given Interview and Skill combination, if you want to save a new Skill Mapping, try using the POST method",
